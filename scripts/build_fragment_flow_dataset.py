...
def process_complex(complex_dir: Path, out_dir: Path, dummy: bool = False) -> dict:
    ...
    # Parse protein
    prot_data = parse_pocket_pdb(pocket_pdb)
    if prot_data is None:
        status["reason"] = "protein_parse_failed"
        return status
    if prot_data["res_coords"].shape[0] == 0:
        status["reason"] = "no_residues"
        return status
...
def main() -> None:
    ...
    log.info("  Success: %d / %d (%.1f%%)", len(successes), len(results), 100 * len(successes) / max(len(results), 1))
    ...
    if successes:
        ...
        log.info("  Residues:  min=%d, median=%d, max=%d", min(n_res), sorted(n_res)[len(n_res) // 2], max(n_res))
        log.info("  Atoms:     min=%d, median=%d, max=%d", min(n_atom), sorted(n_atom)[len(n_atom) // 2], max(n_atom))
        log.info("  Fragments: min=%d, median=%d, max=%d", min(n_frag), sorted(n_frag)[len(n_frag) // 2], max(n_frag))
        log.info("  Single-atom frags: min=%d, median=%d, max=%d", min(n_single), sorted(n_single)[len(n_single) // 2], max(n_single))
