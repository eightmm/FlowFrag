...
class _MultiResInteractionLayer(nn.Module):
    ...
    def __init__(
        ...
        self.l2_dim = vec_dim // 2  # 2e channels (quadrupole)
        l2 = self.l2_dim
        node_irreps = cue.Irreps("O3", f"{scalar_dim}x0e + {vec_dim}x1o + {vec_dim}x1e + {l2}x2e")
...
class DockingHead(nn.Module):
    ...
    def __init__(
        ...
        lifted_irreps = cue.Irreps("O3", f"{hidden_scalar_dim}x0e + {hidden_vec_dim}x1o + {l2}x2e")
        ...
        # --- Output head ---
        l2 = self.l2_dim
        frag_irreps = cue.Irreps("O3", f"{hidden_scalar_dim}x0e + {hidden_vec_dim}x1o + {hidden_vec_dim}x1e + {l2}x2e")
        head_irreps = cue.Irreps("O3", f"{hidden_scalar_dim // 2}x0e + {hidden_vec_dim}x1o + {hidden_vec_dim}x1e + {l2}x2e")
        ...
        atom_irreps = cue.Irreps("O3", f"{hidden_scalar_dim}x0e + {hidden_vec_dim}x1o + {hidden_vec_dim}x1e + {l2}x2e")
    ...
    def forward(
        ...
        # Build fragment hidden: scalar + 1o + 1e + 2e (all non-inplace)
        l2 = self.l2_dim
        # h_lifted has: scalar(scalar_dim) + 1o(vec_dim*3) + 2e(l2*5)
        lift_1o_end = scalar_dim + vec_dim * 3
        lift_2e_end = lift_1o_end + l2 * 5
        ...
        rot_1o = rot_vecs.reshape(n_frag, vec_dim * 3)

        h_frag_0e = h_frag_scalar + h_lifted[:, :scalar_dim]
        h_frag_1o = h_lifted[:, scalar_dim:lift_1o_end] + rot_1o
        h_frag_1e = torch.zeros(n_frag, vec_dim * 3, device=device, dtype=T_frag.dtype)
        h_frag_2e = h_lifted[:, lift_1o_end:lift_2e_end]
        h_frag = torch.cat([h_frag_0e, h_frag_1o, h_frag_1e, h_frag_2e], dim=-1)
        ...
        h_atom_2e = torch.zeros(n_atom, l2 * 5, device=device, dtype=T_frag.dtype)
        h_atom_state = torch.cat([h_atom_scalar, h_atom_1o, h_atom_1e, h_atom_2e], dim=-1)
        ...
        h_prot_2e = torch.zeros(n_prot, l2 * 5, device=device, dtype=T_frag.dtype)
        h_prot_full = torch.cat([h_prot_s, h_prot_1o, h_prot_1e, h_prot_2e], dim=-1)

        # --- Interaction layers with per-layer R_t re-injection ---
        # Recompute gated rotation vectors each layer to prevent orientation dilution
        _1o_start = scalar_dim
        _1o_end = scalar_dim + vec_dim * 3
        for layer in self.interaction_layers:
            ...
            # Re-inject rotation columns as 1o residual (non-inplace for autograd)
            layer_rot_gate = self.frag_rot_gate(h_frag[:, :scalar_dim])
            ...
            h_frag = torch.cat([
                h_frag[:, :_1o_start],
                h_frag[:, _1o_start:_1o_end] + layer_rot_vecs.reshape(n_frag, vec_dim * 3),
                h_frag[:, _1o_end:],
            ], dim=-1)
