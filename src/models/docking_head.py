...
class DockingHead(nn.Module):
    ...
    def forward(
        ...
        # Build fragment hidden: scalar + 1o + 1e + 2e
        l2 = self.l2_dim
        feat_dim = scalar_dim + vec_dim * 3 + vec_dim * 3 + l2 * 5  # 0e + 1o(3) + 1e(3) + 2e(5)
        ...
        h_frag[:, scalar_dim:scalar_dim + vec_dim * 3] = h_lifted[:, scalar_dim:lift_1o_end]
        # 1e stays zero-init (will be generated by TP coupling)
        # 2e from lift
        frag_2e_start = scalar_dim + vec_dim * 3 + vec_dim * 3
        h_frag[:, frag_2e_start:frag_2e_start + l2 * 5] = h_lifted[:, lift_1o_end:lift_2e_end]
        ...
        for layer in self.interaction_layers:
            ...
            # Re-inject rotation columns as 1o residual (keeps orientation anchor)
            ...
            h_frag[:, scalar_dim:scalar_dim + vec_dim * 3] = (
                h_frag[:, scalar_dim:scalar_dim + vec_dim * 3] + layer_rot_vecs.reshape(n_frag, vec_dim * 3)
            )
